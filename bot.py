...
async def handle_url(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # X/Twitter uchun maxsus holat - doim video deb hisoblaymiz
    ...
    # Instagram rasmlar uchun maxsus holat
    is_image = False
    
    # Instagram rasm aniqlash
    if 'instagram.com' in url and ('/p/' in url or '/reel/' in url) and not ('video' in url.lower()):
        is_image = True
    # Pinterest rasm aniqlash
    elif 'pinterest.com' in url and '/pin/' in url:
        is_image = True
    
    if is_image:
        # Rasm havolasi bo'lsa to'g'ridan-to'g'ri yuklab olish
...
async def download_image(update: Update, context: ContextTypes.DEFAULT_TYPE, status_message) -> None:
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # Rasmlarni yuborish - logikani soddalashtiramiz
            images_sent = 0
            
            # Thumbnails bo'lsa (ko'p platformalarda shu)
            if info.get('thumbnails'):
                for thumb in sorted(info['thumbnails'], key=lambda x: x.get('width', 0) if x.get('width') else 0, reverse=True):
                    if thumb.get('url'):
                        try:
                            # Eng katta rasmni olish (boshidan eng katta bo'lgani uchun break qilamiz)
            ...
            if 'entries' in info and info['entries'] and images_sent == 0:
                for entry in info['entries']:
                    if entry.get('thumbnails'):
                        largest_thumb = max(entry['thumbnails'], key=lambda x: x.get('width', 0) if x.get('width') else 0)
...
async def download_media(update: Update, context: ContextTypes.DEFAULT_TYPE, format_type: str, quality: str) -> None:
    ...
    try:
        # X/Twitter uchun maxsus optsiyalar
        if ('twitter.com' in url or 'x.com' in url):
            await update_status_message(context, user_id, status_message, "Twitter/X videosini yuklab olish...", edit_only=True)
        ...
        # Twitter/X uchun maxsus sozlamalar
        if ('twitter.com' in url or 'x.com' in url):
            if format_type == 'audio':
                ...
                # Twitter uchun eng yaxshi format
                ydl_opts['format'] = 'best[ext=mp4]/best'
        else:
            # Boshqa platformalar uchun standart formatlar
            if format_type == 'audio':
                ydl_opts['format'] = 'bestaudio/best'
                ydl_opts['postprocessors'] = [{
                    'key': 'FFmpegExtractAudio',
                ...
                # ffmpeg o'rnatilmagan holatda ham ishlash uchun
                if quality == 'low':
                    ydl_opts['format'] = 'worst[ext=mp4]'
                elif quality == 'medium':
                    # Format birlashtirishni talab qilmaydigan format
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if not os.path.exists(file_path):
                ...
                # Fayl nomi boshida temp_user_id bo'lsa
                    ...
                    if format_type == 'audio':
                        ...
                        if not file_ext or file_ext not in ['.mp4', '.avi', '.mov', '.mkv']:
                            ...
                            try:
                                ...
                                # Faylni qayta ochish kerak
                                file.close()
                                file = open(file_path, 'rb')
                            except Exception as e:
                                logger.error(f"Error renaming file: {e}")
                        
                        # Video parametrlarini qo'shish
                        try:
                            ...
                            file.seek(0)  # Faylni boshidan o'qish
